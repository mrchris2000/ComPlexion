...
namespace Complexion.Portable.Connection
{
    internal sealed class XmlDeserializer
	{
		...
		private object HandleListDerivative(object x, XElement root, string propName, Type type)
		{
			...
			if (type.GetTypeInfo().IsGenericType)
			{
                t = type.GetTypeInfo().GenericTypeArguments[0];
			}
			else
			{
                t = type.GetTypeInfo().BaseType.GenericTypeArguments[0];
			}
	    ...
	    private object GetValueFromXml(XElement root, XName name, PropertyInfo prop)
		{
			object val = null;
